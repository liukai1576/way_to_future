...
export default function Playlist() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      // Handle query param for specific article
  ...
  const handleSortToggle = () => {
    ...
    setPlaylist(sortData(playlist, newOrder));
    const currentId = playlist[currentTrack]?.id;
    const newIndex = sortData(playlist, newOrder).findIndex(a => a.id === currentId);
    if (newIndex !== -1) setCurrentTrack(newIndex);
  };

  const togglePlay = () => {
    if (!isPlaying && playlist.length > 0) {
      // Record listen analytics
      fetch('/api/analytics', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: user.id, article_id: playlist[currentTrack].id, action: 'listen' })
      });
    }
  ...
  const handleMoreClick = () => {
    alert('更多功能（倍速播放、定时关闭）即将上线，敬请期待。');
  };
  ...
  if (playlist.length === 0) {
    return (
      <div className="flex flex-col h-full font-sans bg-neutral-50 p-6">
        <div className="text-center text-neutral-400 mt-20">
          <p className="mb-2">暂无播客内容</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col min-h-full font-sans bg-neutral-50">
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        ...
        <div className="bg-white p-8 pb-12 rounded-b-[40px] shadow-sm mb-6">
          <div className="flex flex-col items-center">
            <div className="w-56 h-56 rounded-full overflow-hidden shadow-2xl mb-8 border-4 border-white relative group">
              <img 
                src={`https://picsum.photos/seed/${currentItem?.id}/400/400`} 
                alt="专辑封面" 
                className={clsx("w-full h-full object-cover transition-transform duration-1000", isPlaying && "animate-spin-slow")}
            ...
            <h3 className="text-xl font-bold text-neutral-900 text-center mb-2 leading-tight px-4">
              {currentItem?.title}
            </h3>
            <p className="text-xs text-neutral-500 mb-8 flex items-center gap-1">
              正在播放：{currentItem?.tags?.[0] || '深度专题'} <span className="text-orange-500">●</span>
            </p>
            ...
            <div className="w-full mb-8 px-4">
              <input 
                ...
                value={currentTime}
                onChange={handleSeek}
                className="w-full h-1 bg-neutral-100 rounded-full mb-2 appearance-none cursor-pointer accent-orange-500"
              />
            ...
            <div className="flex items-center justify-center gap-6 w-full">
              ...
              <button 
                onClick={togglePlay}
                className="w-14 h-14 flex items-center justify-center bg-orange-500 text-white rounded-full shadow-lg shadow-orange-200 hover:bg-orange-600 hover:scale-105 active:scale-95 transition-all"
        ...
        {/* Playlist Section (Lower Section) */}
        <div className="px-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <ListMusic size={18} className="text-orange-500" />
              <h3 className="text-sm font-bold text-neutral-900">播放列表</h3>
              <span className="text-[10px] bg-neutral-200 text-neutral-600 px-1.5 py-0.5 rounded-md">{playlist.length}</span>
            </div>
            <button 
              onClick={handleSortToggle}
              className="flex items-center gap-1 text-xs text-neutral-500 hover:text-orange-500 transition-colors"
            >
              <ArrowUpDown size={14} />
              ...
              <button
                ...
                className={clsx(
                  ...
                  currentTrack === index 
                    ...
                    : "bg-white border border-transparent hover:border-neutral-200"
                )}
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className={clsx(
                    ...
                    currentTrack === index ? "text-orange-600" : "text-neutral-900"
                  )}>
                    {item.title}
                  </h4>
