...
export default function ArticleDetail() {
  ...
  return (
    <div className="bg-white min-h-full pb-24 font-sans">
      {/* Top Nav - Changed from absolute to sticky with background for better visibility and less blocking */}
      ...
      {/* Cover Image - Adjusted margin to account for sticky header */}
      <div className="w-full h-72 relative rounded-b-[40px] overflow-hidden shadow-sm">
        <img 
          src={`https://picsum.photos/seed/${article.id}/800/800`} 
          alt="封面" 
      ...
      <div className="p-6 -mt-10 relative z-10 bg-white rounded-t-[40px]">
        <h1 className="text-2xl font-bold text-neutral-900 leading-snug mb-2 text-center">
          {article.title}
        </h1>
        <p className="text-sm text-neutral-500 mb-6 text-center">
          by {article.tags?.[0] || '作者'}
        </p>
        ...
        <Tabs.Root key={article.id} defaultValue="spectrum" value={activeTab} onValueChange={setActiveTab} className="flex flex-col">
          <Tabs.List className="flex border-b border-neutral-100 mb-6">
            <Tabs.Trigger 
              value="spectrum" 
              className="flex-1 py-3 text-sm font-medium text-neutral-500 data-[state=active]:text-orange-500 data-[state=active]:border-b-2 data-[state=active]:border-orange-500 transition-colors"
            >
              播客
            </Tabs.Trigger>
            <Tabs.Trigger 
          ...
          <Tabs.Content value="spectrum" className="outline-none">
            <div className="bg-neutral-900 rounded-2xl aspect-[16/9] flex flex-col items-center justify-center p-6 relative overflow-hidden group">
              <div className="flex items-end gap-1 h-32 mb-4">
                {[...Array(20)].map((_, i) => (
                  <div 
                    key={i} 
                    className="w-2 bg-orange-500 rounded-t-full animate-[music-bar_1s_ease-in-out_infinite]"
                    style={{ 
            ...
            <div className="mt-4 p-4 bg-neutral-50 rounded-xl border border-neutral-100">
              ...
              <p className="text-xs text-neutral-500 leading-relaxed">
                通过 AI 语音分析，本文的核心关键词集中在：商业模式、大模型、业务创新。音频情绪基调为：专业、前瞻、启发。
              </p>
            </div>
          </Tabs.Content>
          ...
          <Tabs.Content value="infographic" className="outline-none">
            <div className="rounded-2xl overflow-hidden border border-neutral-200 shadow-sm bg-neutral-50 min-h-[300px] flex items-center justify-center">
              {article.infographic_url ? (
                <img src={article.infographic_url} alt="信息图" className="w-full h-auto" referrerPolicy="no-referrer" />
              ) : (
                <div className="text-neutral-400 text-sm">暂未上传信息图</div>
              )}
            </div>
          </Tabs.Content>
          ...
          <Tabs.Content value="summary" className="outline-none">
            <div className="space-y-4">
              ...
              <div className="bg-neutral-100 rounded-2xl overflow-hidden border border-neutral-200 shadow-inner relative group h-[500px]">
                {article.presentation_url && article.presentation_url.trim() !== '' ? (
                  <iframe 
                    src={article.presentation_url} 
              ...
              {article.presentation_url && article.presentation_url.trim() !== '' && (
                <a 
                  href={article.presentation_url} 
                  target="_blank" 
                  rel="noreferrer"
                  className="flex items-center justify-center gap-2 w-full py-4 bg-blue-50 text-blue-600 rounded-xl text-sm font-bold hover:bg-blue-100 transition-all"
                >
          ...
          <Tabs.Content value="text" className="outline-none">
            <div className="prose prose-neutral prose-sm max-w-none">
              {article.content.split(/(?:\r?\n|\\n)+/).filter(Boolean).map((paragraph: string, idx: number) => {
                  ...
                  return (
                    <div key={idx} className="mb-6 relative">
                      <p className="text-neutral-800 leading-relaxed text-[15px]">
                        {parts[0]}
                        <span className="bg-amber-100 text-amber-900 px-1 rounded mx-0.5 border-b border-amber-300">
                          {comment.highlight_text}
                        </span>
                        {parts[1]}
                      </p>
                      <div className="mt-3 bg-neutral-50 border-l-4 border-orange-500 p-3 rounded-r-lg text-sm text-neutral-600 flex gap-3 items-start">
                        <MessageSquare size={16} className="text-orange-500 mt-0.5 shrink-0" />
                        <div>
                          <span className="font-semibold text-orange-900 block mb-1">专家点评</span>
                          {comment.text}
                        </div>
                      </div>
                    </div>
                ...
                return (
                  <p key={idx} className="text-neutral-800 leading-relaxed text-[15px] mb-4">
                    {paragraph}
                  </p>
                );
              })}
        ...
        <div className="fixed bottom-20 left-0 right-0 px-6 z-20 max-w-md mx-auto">
          <div className="bg-white rounded-full shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] p-2 flex items-center justify-between border border-neutral-100">
            ...
            <button 
              ...
              className="px-8 py-3 rounded-full font-bold shadow-md bg-orange-500 text-white hover:bg-orange-600 transition-colors"
            >
              立即听播客
            </button>
          </div>
